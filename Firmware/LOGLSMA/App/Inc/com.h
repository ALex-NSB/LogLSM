...
/* Взвести флаг «несохранённые данные» (стр.122 внутр. Flash), если ещё не
 * взведён. Вызывается из пути записи цикла в NOR Flash (Data.c) — 28.07.2026. */
void data_flag_set_if_clear(void);

/* Маркёр слова текущего формата записи цикла (0xF5 базовый / 0xF3 уплотнённый /
 * 0xF4 подробный). Формат задаётся командой REC_FORMAT (0x31), хранится в ОЗУ.
 * Вызывается из Data.c::SaveParamOnEEPROM. 28.07.2026. */
uint8_t rec_format_marker(void);

/* Сбросить флаг «несохранённые данные» (стереть стр.122). Вызывается при
 * стирании NOR Flash (чип/данные): нет данных → нет несохранённых → флаг сброшен
 * → «Сохранено» зелёный. Иначе флаг рассинхронится с реальными данными. 28.07. */
void data_flag_clear(void);
