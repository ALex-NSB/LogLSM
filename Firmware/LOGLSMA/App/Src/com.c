...
#include "boot.h"        /* договор с загрузчиком: карта Flash, магия BKP_DR3, коды 0x39.. */
...
void cmdBootEnter(LtpPacket *wc);                  /* 0x39 — отдать управление загрузчику (вызов .bootsec), 05.08.2026 */
void cmdIflashRead(LtpPacket *wc);                 /* 0x3E — чтение внутренней Flash STM32 (диагностика), 05.08.2026 */
...
static cmd_callback_t CMD[128] = {
  ...
  cmdGetAxesRaw,                                 /* 0x1A */
    ...
    [0x3E] = cmdIflashRead,                       /* 0x3E  чтение внутренней Flash STM32 (диагностика) — Сервис (05.08.2026) */
    [0x39] = cmdBootEnter                         /* 0x39  отдать управление загрузчику (.bootsec) — Сервис (05.08.2026).
                                                   * 0x3A..0x3D (BEGIN/DATA/END/GO) приложение НЕ знает: на них
                                                   * отвечает только загрузчик. См. LOGLSMA/App/Inc/boot.h. */
};
...
#define FW_YY  26   /* год  */
#define FW_MM   8   /* мес  */
#define FW_DD   6   /* число*/
#define FW_HH   0   /* часы */
#define FW_MI  30   /* мин  — АВАРИЙНЫЙ ВЕКТОР: на время заливки в стр.0 лежит вектор на загрузчик, обрыв питания больше
                     * не требует SWD (правка в boot_loader.c). Ранее: LTP 0x3E чтение внутр. Flash. 06.08.2026.
                     * ЗАГРУЗЧИК ВНУТРИ ЭТОГО ЖЕ ОБРАЗА: секция .bootsec (стр.113..120, 0x08038800), вход по
                     * LTP 0x39 вызовом через указатель; база приложения НЕ менялась (0x08000000), длина 226 КБ.
                     * Прошивка одна, отдельного проекта загрузчика больше нет. 05.08.2026.
                     * ⚠ 04.08 версия на диске стояла как 26.08.03 10:09 (FW_DD не подняли при правке от 04.08) — расхождение
                     *   выглядело как несвежая сборка. Здесь исправлено. */
...
/* 0x39 — отдать управление загрузчику (05.08.2026). Обработчик живёт в таблице
 * команд, то есть отработать может только из Service() — а туда попадают лишь
 * по кабелю. Отдельный гейт «только Сервис» поэтому не нужен: без кабеля эта
 * команда физически не доедет.
 *
 * Загрузчик — не отдельная прошивка, а часть ЭТОГО же образа, уложенная
 * линкером в .bootsec (стр.113..120) и не перезаписываемая при обновлении по
 * кабелю. Поэтому переход — обычный вызов функции, без прыжков через таблицу
 * векторов. Обратно загрузчик уходит сбросом (его команда GO), и приложение
 * стартует с чистого железа.
 *
 * ⚠ Зовём по указателю из первого слова секции, а НЕ по имени функции: по
 * кабелю обновляется только приложение, а .bootsec остаётся от прошивки по
 * SWD — адрес из текущей сборки мог бы не совпасть с тем, что реально лежит.
 * Правило 3 в boot.h.
 *
 * Что обязательно погасить перед вызовом: DMA-приём UART (иначе он продолжит
 * писать в память по своим настройкам), SysTick и вообще все прерывания —
 * таблица векторов лежит в стираемой области, прилетевшее прерывание ушло бы
 * в никуда. IWDG не гасим: программно он не выключается, его гладит сам
 * загрузчик. */
/* 0x3E — прочитать кусок ВНУТРЕННЕЙ Flash STM32 (05.08.2026).
 *
 * Зачем: до этого внутреннюю память нельзя было посмотреть иначе как
 * программатором. А смотреть есть на что — там и паспорт с калибровками
 * (стр.123), и журналы (121, 124..127), и секция загрузчика (113..120), и сам
 * код приложения. Для диагностики «что реально лежит в приборе» этого не
 * хватало: вкладка «Тест памяти» читает ВНЕШНИЙ NOR, к внутренней Flash
 * отношения не имеющий (и страницы у них разные: 256 Б против 2 КБ).
 *
 * Только чтение — записи по этому пути нет и не будет: внутренняя Flash
 * правится исключительно своими командами (паспорт, калибровки, журналы),
 * каждая со своей проверкой.
 *
 * payload: [0..3] адрес u32 LE, [4..5] длина u16 LE (≤256)
 * ответ:   [0] код ошибки, далее сами байты. */
void cmdIflashRead(LtpPacket *wc)
{
  uint8_t err;
  if (wc->n < 6) { err = er_badarg; sendPacket(wc->addr, wc->cmd, &err, 1); return; }

  uint32_t addr; memcpy(&addr, &wc->data[0], 4);
  uint16_t len;  memcpy(&len,  &wc->data[4], 2);
  if (len > 256u) len = 256u;

  /* Диапазон — вся внутренняя Flash 256 КБ и ни байтом больше: чтение мимо
   * (периферия, чужие адреса) в лучшем случае бессмысленно, в худшем — HardFault. */
  if (len == 0 || addr < 0x08000000u || (addr + len) > 0x08040000u)
  { err = er_badarg; sendPacket(wc->addr, wc->cmd, &err, 1); return; }

  uint8_t ans[1 + 256];
  ans[0] = er_none;
  memcpy(&ans[1], (const void *)addr, len);
  sendPacket(wc->addr, wc->cmd, ans, 1u + len);
}

void cmdBootEnter(LtpPacket *wc)
{
  /* Загрузчик на месте? Указатель обязан смотреть внутрь своей же секции. */
  const uint32_t entry = *(volatile uint32_t *)BOOT_ENTRY_PTR_ADDR;
  if (entry < BOOT_SEC_BASE || entry >= (BOOT_SEC_BASE + BOOT_SEC_SIZE))
  {
    uint8_t err = er_not_impl;        /* секция пуста — обновляться нечем */
    sendPacket(wc->addr, wc->cmd, &err, sizeof(err));
    return;
  }

  uint8_t ok = er_none;
  sendPacket(wc->addr, wc->cmd, &ok, sizeof(ok));
  HAL_Delay(50);                      /* дать ответу дойти до ПК */

  HAL_UART_Abort(&huart2);            /* остановить DMA-приём/передачу... */
  HAL_UART_DeInit(&huart2);           /* ...и снять настройку целиком */

  __disable_irq();
  SysTick->CTRL = 0;
  SysTick->LOAD = 0;
  SysTick->VAL  = 0;
  for (int i = 0; i < 8; i++)         /* запретить и снять все NVIC-прерывания */
  {
  ...
  ((void (*)(void))entry)();
  while (1) { }                       /* сюда не возвращаются: выход — сброс */
}
