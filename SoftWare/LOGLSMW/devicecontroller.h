...
constexpr quint8 SYNC_REF_SET       = 0x37;   // опорная точка синхро: задать t0 (backup DR1)
constexpr quint8 SYNC_REF_GET       = 0x38;   // опорная точка синхро: прочитать t0 + валидность
/* --- Обновление прошивки по кабелю (05.08.2026) ---------------------------
 * 0x39 понимают ОБА: приложение (отдаёт управление загрузчику прыжком, без
 * сброса) и сам загрузчик (отвечает «я тут»). 0x3A..0x3D — только загрузчик.
 * Различить, кто на линии, можно по ответу на 0x39/WHO_AM_I: байт [0] = 0xB0 у
 * загрузчика, WHO_AM_I датчика (0x6C/0x70) у приложения.
 * Полный договор — Firmware/LOGLSMA/App/Inc/boot.h. */
constexpr quint8 BOOT_ENTER         = 0x39;   // приложение → прыжок в загрузчик (стр.113, 0x08038800)
constexpr quint8 BOOT_BEGIN         = 0x3A;   // [0..3] размер LE, [4..7] CRC32 LE → стирание (долгий ответ!)
constexpr quint8 BOOT_DATA          = 0x3B;   // [0..3] смещение LE (кратно 8), далее ≤256 байт образа
constexpr quint8 BOOT_END           = 0x3C;   // сверить CRC32 записанного образа
constexpr quint8 BOOT_GO            = 0x3D;   // ответ и сброс → стартует новое приложение
constexpr quint8 BOOT_WHOAMI_MARK   = 0xB0;   // метка «отвечает загрузчик» в байте [0]
constexpr int    BOOT_CHUNK         = 256;    // размер куска DATA (кратен 8)

/* Чтение ВНУТРЕННЕЙ Flash STM32 (05.08.2026) — только диагностика, записи по
 * этому пути нет. ⚠ Не путать с FLASH_READ (0x07): тот читает ВНЕШНИЙ NOR (чип
 * данных, страница 256 Б), а здесь память самого МК — страница 2 КБ, в ней код,
 * паспорт (стр.123), журналы (121, 124..127) и секция загрузчика (113..120).
 * payload: [0..3] адрес u32 LE, [4..5] длина u16 LE (≤256); ответ: [0]=err + данные. */
constexpr quint8 IFLASH_READ        = 0x3E;
