#include "com.h"
#include "wake.h"
//#include "FlashQ.h"
#include "main.h"
#include "quadspi.h"
#include "fm25xx.h"
#include "spi.h"
#include "tmp117.h"
#include "i2c.h"
#include "string.h"
#include "lsm6dso_temp.h"
#include "stm_temp.h"

#include "usart.h"
#include "dma.h"
#include "rtc.h"

#include "stdarg.h"
#include "string.h"
#include "stdio.h"
#include "stdlib.h"
#include "stdint.h"

#define WORD_PAR(X, Y)          (X << 8 | Y)

typedef void (*cmd_callback_t)(WakeContext *wc);

/* errors */
enum {er_none, er_timeout, er_badarg, er_not_impl, er_dead_imu};

enum MemoryType {m_eeprom, m_fram};

void invoke_cmd_handler(WakeContext *wc);
void sendPacket(uint8_t addr, uint8_t cmd, void *data, uint32_t n);
void ReadReceiveBuffer(UART_HandleTypeDef *huart, uint32_t start, uint32_t stop);

void cmdFlashReadIDHandler(WakeContext *wc);            //C0 8D 04 00 00
void cmdFlashChipEraseHandler(WakeContext *wc);
void cmdFlashPageEraseHandler(WakeContext *wc);         
void cmdFlashWritePageHandler(WakeContext *wc);         //C0 8D 05 00 00 00 00 00
void cmdFlashReadHandler(WakeContext *wc);              //C0 8D 07 00 08 00 00 00 00 00 01 00 00
void cmdFlashGetState(WakeContext *wc);                 //C0 8D 09 00 00
void cmdGetTemp(WakeContext *wc);
void cmdSetBaud(WakeContext *wc);
void cmdFramWritePageHandler(WakeContext *wc);
void cmdFramReadHandler(WakeContext *wc);
void cmdFlashOn(WakeContext *wc);
void cmdFlashOff(WakeContext *wc);
void cmdGetTempIMU(WakeContext *wc);
void cmdGetTempChip(WakeContext *wc);
//void cmdSetAcquisitionData(WakeContext *wc);
//void cmdGetAcquisitionData(WakeContext *wc);

void cmdAccSetOdr(WakeContext *wc);
void cmdAccSetFullScale(WakeContext *wc);
void cmdGyroSetOdr(WakeContext *wc);
void cmdGyroSetFullScale(WakeContext *wc);
void cmdAccGetOdr(WakeContext *wc);
void cmdAccGetFullScale(WakeContext *wc);
void cmdGyroGetOdr(WakeContext *wc);
void cmdGyroGetFullScale(WakeContext *wc);
void cmdGetAxesRaw(WakeContext *wc);


void cmdSetDateTime(WakeContext *wc);
void cmdGetDateTime(WakeContext *wc);

void cmdStartRegister(WakeContext *wc);

static cmd_callback_t CMD[128] = {
  NULL, NULL, NULL, NULL,                       // 0x0 0x1 0x2 0x3
  cmdFlashReadIDHandler,                        // 0x4
  cmdFlashChipEraseHandler,                     // 0x5
  cmdFlashPageEraseHandler,                     // 0x6
  cmdFlashReadHandler,                          // 0x7
  cmdFlashWritePageHandler,                     // 0x8
  cmdFlashGetState,                             //0x9
  cmdGetTemp,                                   //0xA
  cmdSetBaud,                                   //0xB
  cmdFramWritePageHandler,                      /* 0xC  */
  cmdFramReadHandler,                           /* 0xD  */
  cmdFlashOn,                                   /* 0xE  */
  cmdFlashOff,                                  /* 0xF  */
  cmdGetTempIMU,                                /* 0x10 */
  cmdGetTempChip,                               /* 0x11 */
  cmdAccSetOdr,                                 /* 0x12 */
  cmdAccSetFullScale,                           /* 0x13 */
  cmdGyroSetOdr,                                /* 0x14 */
  cmdGyroSetFullScale,                          /* 0x15 */
  cmdAccGetOdr,                                 /* 0x16 */
  cmdAccGetFullScale,                           /* 0x17 */
  cmdGyroGetOdr,                                /* 0x18 */
  cmdGyroGetFullScale,                          /* 0x19 */
  cmdGetAxesRaw,                                 /* 0x1A */
    cmdGetDateTime,                             /* 0x1B */
    cmdSetDateTime,                              /* 0x1C */
    cmdStartRegister                            /* 0x1D */
  /* NULL */
};
  
static WakeContext wc;
static uint8_t TxBuf[1024];
static uint8_t RxBuf[512];
static uint32_t prevFill;
//static uint32_t TxSize;
static uint8_t FlashChipHasErased;

//lsm
/*
static struct AcqusitionDataStruct
{
  LSM6DSO_AxesRaw_t acc;
  LSM6DSO_AxesRaw_t gyro;
  int32_t rot;
} AcqusitionData;*/

static int32_t rot;
static uint32_t N;
static uint8_t QUIT;
//extern UART_HandleTypeDef huart2;

void ComInit()
{
  HAL_UART_DeInit(&huart2);
  MX_DMA_Init();
  MX_USART2_UART_Init();
  
  N = 0;
  rot = 0;
  QUIT = 0;
  
  WakeProtocolInit(&wc);
  prevFill = 0;
  FlashChipHasErased = 0;
  HAL_UARTEx_ReceiveToIdle_DMA(&huart2, RxBuf, sizeof(RxBuf));
  //HAL_UART_Receive_IT(&huart2, RxBuf, sizeof(RxBuf));
}

void ComDeInit()
{
  HAL_UART_DeInit(&huart2);
}

void Service()
{
  ComInit();
  
  uint8_t *info = "COM start\r\n";
  HAL_UART_Transmit(&huart2, info, strlen(info), 1000);
  
  while(!QUIT)
  {
    ComPoll();
    
    ///условие выхода из сервиса
    uint32_t t = HAL_GetTick();
    while(GPIO_PIN_RESET == HAL_GPIO_ReadPin(WKUP1_GPIO_Port, WKUP1_Pin))
    {
      if((HAL_GetTick() - t) > 3000)
      {
          uint8_t *info = "exit\r\n";
          HAL_UART_Transmit(&huart2, info, strlen(info), 1000);
        ///деинициализация и выход
        //ComDeInit();
        //__HAL_PWR_CLEAR_FLAG(PWR_FLAG_WUF1);
        return;
      }
    }
  }
}

void ComPoll()
{
  if(wc.packet_recognized)
  {
    if(wc.addr == WAKE_DEV_ADDRESS)
    {
      invoke_cmd_handler(&wc);
      wc.packet_recognized = 0;
    }
  }
}

void invoke_cmd_handler(WakeContext *wc)
{
  if(wc->cmd < sizeof(CMD)/sizeof(CMD[0]))
  {
    if(CMD[wc->cmd] != NULL)
      CMD[wc->cmd](wc);
  }
}

void cmdStartRegister(WakeContext *wc)
{
  uint8_t err = er_none;
  QUIT = 1;
  //sendPacket(wc->addr, wc->cmd, &err, sizeof(err));
}

void cmdSetDateTime(WakeContext *wc)
{
  uint8_t err = er_badarg;
  if(wc->n == sizeof(RTC_DateTime))
  {
    RTC_DateTime *dt = (RTC_DateTime*)wc->data;
    RTC_SetTimeDate(dt);
    err = er_none;
    
    /*RTC_DateTime dt2;
    RTC_GetTimeDate(&dt2);
    if(0 != memcmp(dt, &dt2, sizeof(RTC_DateTime)))
    {
      err = 128;
    }*/
  }
  sendPacket(wc->addr, wc->cmd, &err, sizeof(err));
}

void cmdGetDateTime(WakeContext *wc)
{
  DateTimeResponse resp;
  RTC_GetTimeDate(&resp.datetime);
  resp.cod = er_none;
  sendPacket(wc->addr, wc->cmd, (uint8_t*)&resp, sizeof(resp));
}

void cmdAccSetOdr(WakeContext *wc)
{
  uint8_t err = er_not_impl;
  sendPacket(wc->addr, wc->cmd, &err, 1);
}

void cmdGyroSetOdr(WakeContext *wc)
{
  uint8_t err = er_not_impl;
  sendPacket(wc->addr, wc->cmd, &err, 1);
}

void cmdAccSetFullScale(WakeContext *wc)
{
  uint8_t err = er_none;
  int32_t *fullscale = (int32_t*)wc->data;
  if(wc->n == sizeof(fullscale))
  {
    switch(*fullscale)
    {
    case 2:
    case 4:
    case 8:
    case 16:
      LSM6DSO_ACC_SetFullScale(&lsm, *fullscale);
      break;
    default:
      err = er_badarg;
    }
  }
  else
  {
    err = er_badarg;
  }
  sendPacket(wc->addr, wc->cmd, &err, sizeof(err));
}

void cmdGyroSetFullScale(WakeContext *wc)
{
  uint8_t err = er_none;
  int32_t *fullscale = (int32_t*)wc->data;
  if(wc->n == sizeof(fullscale))
  {
    switch(*fullscale)
    {
    case 125:
    case 250:
    case 500:
    case 1000:
    case 2000:
      LSM6DSO_GYRO_SetFullScale(&lsm, *fullscale);
      break;
    default:
      err = er_badarg;
    }
  }
  else
  {
    err = er_badarg;
  }
  sendPacket(wc->addr, wc->cmd, &err, sizeof(err));
}

void cmdAccGetOdr(WakeContext *wc)
{
  uint8_t err = er_not_impl;
  sendPacket(wc->addr, wc->cmd, &err, 1);
}

void cmdGyroGetOdr(WakeContext *wc)
{
  uint8_t err = er_not_impl;
  sendPacket(wc->addr, wc->cmd, &err, 1);
}

void cmdAccGetFullScale(WakeContext *wc)
{
  struct FullScale fs = { 0 };
  LSM6DSO_ACC_GetFullScale(&lsm, &fs.fullscale);
  sendPacket(wc->addr, wc->cmd, &fs, sizeof(fs));
}

void cmdGyroGetFullScale(WakeContext *wc)
{
  struct FullScale fs = { 0 };
  LSM6DSO_GYRO_GetFullScale(&lsm, &fs.fullscale);
  sendPacket(wc->addr, wc->cmd, &fs, sizeof(fs));
}

void cmdGetAxesRaw(WakeContext *wc)
{
  struct AxesRaw axes = { 0 };
  LSM6DSO_ACC_GetAxesRaw(&lsm, &axes.acc);
  LSM6DSO_GYRO_GetAxesRaw(&lsm, &axes.gyro);
  sendPacket(wc->addr, wc->cmd, &axes, sizeof(axes));
}

void printff(const char *format, ...)
{
  if(format != NULL)
  {
    va_list args1;
    va_start(args1, format);
    int len = vsnprintf(TxBuf, sizeof(TxBuf), format, args1);
    va_end(args1);
    HAL_UART_Transmit(&huart2, TxBuf, len, 1000);
  }
}

void print(char *str)
{
  HAL_UART_Transmit(&huart2, str, strlen(str), 1000);
}

void printHex(uint8_t *src, uint32_t size, uint32_t width)
{
  for(uint32_t i=0; i<size; i++)
  {
    printff("%02X ", src[i]);
    if(((i+1) % width) == 0)
      print("\r\n");
  }
}

uint32_t send(uint8_t *buff, uint32_t size)
{
  return HAL_UART_Transmit(&huart2, buff, size, 1000);
}

/*void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  if(GPIO_Pin == INT1_Pin)
  {
    uint8_t Status1, Status2;
    LSM6DSO_ACC_Get_DRDY_Status(&lsm, &Status1);
    LSM6DSO_GYRO_Get_DRDY_Status(&lsm, &Status2);
    
    if(Status1 || Status2)
    {
      LSM6DSO_ACC_GetAxesRaw(&lsm, &AcqusitionData.acc);
      LSM6DSO_GYRO_GetAxesRaw(&lsm, &AcqusitionData.gyro);
      rot += AcqusitionData.gyro.y;
      N++;
      if(N >= 10)
      {
        rot /= N;
        ///сохранить rot для считывания
        AcqusitionData.rot = rot;
        rot = 0;
        N = 0;
      }
    }
  }
}

void cmdSetAcquisitionData(WakeContext *wc)
{
  GPIO_InitTypeDef GPIO_InitStruct = {0};
  uint8_t err = er_none;
  uint8_t imuEnable = wc->data[0];
  if(imuEnable != 0)
  {
    ///Проверка датчика
    uint8_t id;
    LSM6DSO_ReadID(&lsm, &id);
    if(id != LSM6DSO_ID)
    {
      err = er_dead_imu;
      sendPacket(wc->addr, wc->cmd, &err, sizeof(err));
      return;
    }
    
    ///Настройка датчика
    LSM6DSO_DRDY_Set_Mode(&lsm, LSM6DSO_DRDY_PULSED);
    LSM6DSO_ACC_Enable_DRDY_On_INT1(&lsm);
    
    ///Прерывание
    GPIO_InitTypeDef GPIO_InitStruct = {0};
    GPIO_InitStruct.Pin = INT1_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
    GPIO_InitStruct.Pull = GPIO_PULLDOWN;
    HAL_GPIO_Init(INT1_GPIO_Port, &GPIO_InitStruct);
    
    HAL_NVIC_SetPriority(EXTI15_10_IRQn, 0, 0);
    HAL_NVIC_EnableIRQ(EXTI15_10_IRQn);
  }
  else
  {
    HAL_GPIO_DeInit(INT1_GPIO_Port, INT1_Pin);
    HAL_NVIC_DisableIRQ(EXTI15_10_IRQn);
    LSM6DSO_ACC_Disable_DRDY_On_INT1(&lsm);
  }
  sendPacket(wc->addr, wc->cmd, &err, sizeof(err)); 
}

void cmdGetAcquisitionData(WakeContext *wc)
{
  struct AcqusitionDataStruct data;
  
  HAL_NVIC_DisableIRQ(EXTI15_10_IRQn);
  memcpy(&data, &AcqusitionData, sizeof(data));
  HAL_NVIC_EnableIRQ(EXTI15_10_IRQn);
  sendPacket(wc->addr, wc->cmd, &data, sizeof(data));
}*/

void cmdFlashOn(WakeContext *wc)
{
  uint8_t f;
  FlashOn();
  P25Qx_SetQPI(&flash);
  //flash.mode = qpi_mode;
  
  f = er_none;
  sendPacket(wc->addr, wc->cmd, &f, 1);
}

void cmdFlashOff(WakeContext *wc)
{
  uint8_t f;
  FlashOff();
  f = er_none;
  sendPacket(wc->addr, wc->cmd, &f, 1);
}

void cmdReadMem(enum MemoryType mem, WakeContext *wc)
{
  uint8_t page[257];
  
  if(wc->n < 8)
    return;
  
  uint32_t addr = *(uint32_t*)(wc->data + 0);
  uint32_t size = *(uint32_t*)(wc->data + 4);
  addr &= 0x00FFFFFF;
  if(size > 256)
    size = 256;
  
  page[0] = er_none;
  
  if(mem == m_eeprom)
    P25Qx_QPI_Read(&flash, addr, size, &page[1]);
  else
    fm25xx_readMultiple(&hspi1, addr, &page[1], size);
  
  sendPacket(wc->addr, wc->cmd, page, 1+size);
}

void cmdWriteMem(enum MemoryType mem, WakeContext *wc)
{
  if(wc->n < 2)
    return;
  
  uint16_t pg_num = *(uint16_t*)(wc->data + 0);
  
  uint32_t nWrite = wc->n-2;
  if(nWrite > 256)
    nWrite = 256;
  
  if(mem == m_eeprom)  
    //FlashPageProgram(&wc->data[2], (pg_num << 8), nWrite);
    P25Qx_QPI_ProgramPage(&flash, (pg_num << 8), &wc->data[2], nWrite);
  else
    fm25xx_writeMultiple(&hspi1, (pg_num << 8), &wc->data[2], nWrite);
  
  uint8_t f = er_none;
  sendPacket(wc->addr, wc->cmd, &f, 1);
}

void cmdFramWritePageHandler(WakeContext *wc)
{
  cmdWriteMem(m_fram, wc);
}

void cmdFramReadHandler(WakeContext *wc)
{
  cmdReadMem(m_fram, wc);
}

void cmdSetBaud(WakeContext *wc)
{
  uint8_t err = er_none;
  if(wc->n < 4)
    return;
  
  uint32_t newBaud = *(uint32_t*)(wc->data + 0);
  sendPacket(wc->addr, wc->cmd, &err, sizeof(err));
  
  HAL_UART_DMAStop(&huart2);
  HAL_UART_DeInit(&huart2);
  huart2.Init.BaudRate = newBaud;
  HAL_UART_Init(&huart2);
  HAL_UARTEx_ReceiveToIdle_DMA(&huart2, RxBuf, sizeof(RxBuf));
}

void cmdGetTemp(WakeContext *wc)
{
  uint8_t ans[5];
  float temp = tmp117_get_Temp(&hi2c2);
  ans[0] = er_none;
  memcpy(&ans[1], &temp, 4);
  sendPacket(wc->addr, wc->cmd, ans, sizeof(ans));
}

void cmdGetTempIMU(WakeContext *wc)
{
  uint8_t ans[5];
  float temp;
  LSM6DSO_GetTemp(&lsm, &temp);
  ans[0] = er_none;
  memcpy(&ans[1], &temp, 4);
  sendPacket(wc->addr, wc->cmd, ans, sizeof(ans));
}
/////////////////////////////////////////////////////////////////////////////////
void cmdGetTempChip(WakeContext *wc)
{
  uint8_t ans[5];
  float temp;
  get_stm_temp(&temp);
  ans[0] = er_none;
  memcpy(&ans[1], &temp, 4);
  sendPacket(wc->addr, wc->cmd, ans, sizeof(ans));
}

void cmdFlashReadIDHandler(WakeContext *wc)
{
  uint32_t id = P25Qx_QPI_ReadID(&flash);
  uint8_t *pID = (uint8_t*)&id;
  uint8_t ans[4] = {er_none, pID[0], pID[1], pID[2]};
  sendPacket(wc->addr, wc->cmd, &ans, sizeof(ans));
}

void cmdFlashGetState(WakeContext *wc)
{
  union StatusRegBits st;
  st.reg[1] = P25Qx_ReadSR(&flash, 1);
  uint8_t ans[] = {er_none, st.flag.WIP};
  sendPacket(wc->addr, wc->cmd, &ans, sizeof(ans));
}

void cmdFlashChipEraseHandler(WakeContext *wc)
{
  uint8_t f = er_none;
  
  P25Qx_QPI_EraseChip(&flash);
  
  sendPacket(wc->addr, wc->cmd, &f, 1);
}

void cmdFlashReadHandler(WakeContext *wc)
{
  cmdReadMem(m_eeprom, wc);
}

void cmdFlashWritePageHandler(WakeContext *wc)
{
  cmdWriteMem(m_eeprom, wc);
}

void cmdFlashPageEraseHandler(WakeContext *wc)
{
  uint8_t f = er_none;
  
  if(wc->n < 2)
    return;
  
  uint16_t pg_num = *(uint16_t*)(wc->data + 0);
  
  P25Qx_QPI_ErasePage(&flash, pg_num << 8);
  sendPacket(wc->addr, wc->cmd, &f, 1);
}

////////////////////////////////////////////////////////////////////////////////
/*void HAL_UART_RxHalfCpltCallback(UART_HandleTypeDef *huart)
{
  ReadReceiveBuffer(huart, prevFill, sizeof(RxBuf)/2-prevFill);
  prevFill = sizeof(RxBuf)/2;
}

void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ReadReceiveBuffer(huart, prevFill, sizeof(RxBuf)-prevFill);
  prevFill = 0;
}*/

/*void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  HAL_UART_Receive_IT(&huart2, RxBuf, sizeof(RxBuf));
}*/

void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t fill)
{
  if(prevFill < fill)
    ReadReceiveBuffer(huart, prevFill, fill-prevFill);
  else
    ReadReceiveBuffer(huart, 0, fill);
  prevFill = fill;
}

void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
  HAL_UARTEx_ReceiveToIdle_DMA(&huart2, RxBuf, sizeof(RxBuf));
}

void ReadReceiveBuffer(UART_HandleTypeDef *huart, uint32_t start, uint32_t size)
{
  for(uint32_t i=0; i<size; i++)
    WakeProtocolParse(&wc, RxBuf[start+i]);
  //HAL_UART_Transmit(&huart2, &RxBuf[start], size, 1000);
}
////////////////////////////////////////////////////////////////////////////////

void sendPacket(uint8_t addr, uint8_t cmd, void *data, uint32_t n)
{
  uint32_t size = WakeProtocolBuildPacket(addr, cmd, data, n, TxBuf);
  HAL_UART_Transmit(&huart2, TxBuf, size, 1000); 
}
